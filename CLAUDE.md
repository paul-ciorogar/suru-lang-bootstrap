...
5b. **Run + collect** (test mode only) — [Testing/](src/Suru.Compiler/Testing/) holds the `#` directive machinery downstream of codegen. The emitted binary prints one record per `#view`/`#assert`, spelled the way [RecordProtocol](src/Suru.Compiler/Testing/RecordProtocol.cs) says — the only place the wire encoding appears, asked by codegen and by the reader alike. [RecordReader](src/Suru.Compiler/Testing/RecordReader.cs) splits records out of stdout and decodes each into a [TestRecord](src/Suru.Compiler/Testing/TestRecord.cs); [TestRun](src/Suru.Compiler/Testing/TestRun.cs) takes it from there — matching each to its directive by id and rewriting the source line — and knows nothing about how a record travelled. A line carries one directive — a directive ends at its terminator (the `:` of a `#view`, the `)` of a `#assert`) and the rest of the line is discarded without being lexed, so a second `#` on the line is text the first one owns. `#mock` has no annotation and so no discard: its expression ends it, and anything after that on the line is an error. `BuildMode` is decided in the **lexer**: `Production` skips a `#` line exactly like `//`, so no later stage can be broken by a directive it does not understand.
